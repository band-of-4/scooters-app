class RestoreService
  ...
  class ValidationError < RestoreError; end
  ...
  def self.restore_from_file(filepath)
    ...
    raise RestoreError, "File not found: #{filepath}" unless File.exist?(filepath)
    ...
    ActiveRecord::Base.transaction do
        ...
        # Сначала считаем текущие данные
        puts "Before clear:"
        ...
        # Хэши для соответствия старых ID новым
        @client_id_mapping = {}
        @scooter_id_mapping = {}
  ...
  when '.yaml', '.yml'
    ...
    raise RestoreError, "Unsupported file format: #{extension}"
  end
  
  result
rescue => e
  puts "Parse error: #{e.message}"
  raise RestoreError, "Failed to parse file: #{e.message}"
end
  
  def self.validate_data_structure(data)
    raise RestoreError, "Invalid backup file structure" unless data.is_a?(Hash)
    
    required_keys = [:clients, :scooters, :rentals]
    missing_keys = required_keys - data.keys
    
    unless missing_keys.empty?
      raise RestoreError, "Missing required data sections: #{missing_keys.join(', ')}"
    end
  end
  ...
  clients_data.each_with_index do |client_attrs, index|
    ...
    # Преобразуем date_of_birth из строки обратно в Date, если нужно
  ...
  rentals_data.each_with_index do |rental_attrs, index|
    ...
    # Находим новые ID для связанных записей
    ...
    unless new_client_id && new_scooter_id
      raise RestoreError, "Cannot find client or scooter for rental. Client mapping: #{@client_id_mapping.inspect}, Scooter mapping: #{@scooter_id_mapping.inspect}"
    end
    ...
    unless client && scooter
      raise RestoreError, "Cannot find client or scooter for rental (after mapping)"
    end
    
    # ВАЖНО: Временно обновляем статус самоката, чтобы пройти валидацию
    # Сохраняем оригинальный статус
    original_status = scooter.status
    
    # Если статус в бекапе 'rented', меняем на 'available' для прохождения валидации
    # а потом вернем обратно
    ...
    begin
      ...
      # Восстанавливаем оригинальный статус самоката
      if original_status != scooter.status
        scooter.update_column(:status, original_status)
      end
      
    rescue => e
      # В случае ошибки все равно восстанавливаем статус
      if original_status != scooter.status
        scooter.update_column(:status, original_status)
      end
      raise e
